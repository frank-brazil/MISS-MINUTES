...
const MissMinutesChat = (() => {
    ...
    function _mockMic() {
        if (_micBtn.classList.contains('active')) {
            _micBtn.classList.remove('active');
            MissMinutesAvatar.setState('idle');
            MissMinutesSidebar.log('Listening stopped (mock)', 'warning');
            const mockText = '[Voice input simulated]';
            _addMessage('system', 'Microphone is in mock mode. Type your message instead.');
        } else {
            _micBtn.classList.add('active');
            MissMinutesAvatar.setState('listening');
            MissMinutesSidebar.log('Listening... (mock)', 'info');
            _addMessage('system', 'Microphone is in mock mode. No real STT is connected.');
            setTimeout(() => {
                _micBtn.classList.remove('active');
                MissMinutesAvatar.setState('idle');
            }, 2000);
