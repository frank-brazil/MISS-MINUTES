...
const MissMinutesAvatar = (() => {
    ...
    function setState(newState) {
        ...
        const sublabel = document.getElementById('avatar-state-sublabel');
        if (sublabel) {
            sublabel.textContent = STATE_SUBLABELS[newState] || '';
        }

        _updateAssistantStatus(newState);
        MissMinutesAvatarAssets.applyState(newState);
    ...
    function _updateAssistantStatus(state) {
        const dot = document.getElementById('status-dot-main');
        const label = document.getElementById('status-label-main');
        const sublabel = document.getElementById('status-sublabel-main');
        if (dot) {
            dot.className = 'assistant-status-dot';
            if (state !== 'idle') dot.classList.add(state);
        }
        if (label) label.textContent = STATE_LABELS[state] || state;
        if (sublabel) sublabel.textContent = STATE_SUBLABELS[state] || '';
    }
