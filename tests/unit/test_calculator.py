...
def test_addition() -> None:
    tool = CalculatorTool()
    result = asyncio.run(tool.execute(operation="add", a=2, b=3))
...
def test_subtraction() -> None:
    tool = CalculatorTool()
    result = asyncio.run(tool.execute(operation="subtract", a=10, b=4))
    assert result.success is True
...
def test_multiplication() -> None:
    tool = CalculatorTool()
    result = asyncio.run(tool.execute(operation="multiply", a=6, b=7))
    assert result.success is True
...
def test_division() -> None:
    tool = CalculatorTool()
    result = asyncio.run(tool.execute(operation="divide", a=20, b=5))
    assert result.success is True
...
def test_division_returns_float_for_non_exact() -> None:
    tool = CalculatorTool()
    result = asyncio.run(tool.execute(operation="divide", a=1, b=3))
    assert result.success is True
...
def test_operation_accepts_enum_and_string() -> None:
    tool = CalculatorTool()
    enum_result = asyncio.run(tool.execute(operation=CalculatorOperation.ADD, a=1, b=2))
    string_result = asyncio.run(tool.execute(operation="add", a=1, b=2))
...
def test_division_by_zero_rejected() -> None:
    tool = CalculatorTool()
    result = asyncio.run(tool.execute(operation="divide", a=10, b=0))
    assert isinstance(result, ToolResult)
...
def test_negative_numbers() -> None:
    tool = CalculatorTool()
    result = asyncio.run(tool.execute(operation="add", a=-5, b=3))
    assert result.success is True
...
def test_decimal_operands() -> None:
    tool = CalculatorTool()
    result = asyncio.run(tool.execute(operation="multiply", a=1.5, b=2.5))
    assert result.success is True
...
def test_risk_expression_string_rejected() -> None:
    tool = CalculatorTool()
    with pytest.raises(ValidationError):
        asyncio.run(tool.execute(operation="add", a="__import__('os')", b=1))
